...
console.log('\nModern barriers');
{
  for (const key of ['tecpro', 'tecpro_low', 'bridge']) {
    const def = LIBRARY.find((d) => d.key === key);
    check(`the library has ${key}`, !!def && def.surface === 'WALL',
      def ? `surface ${def.surface}` : 'missing');
  }
  /* A run tiles by the BODY box, so a module whose blocks stop short of its
     own label tiles at a pitch no snap step divides. Held to the metre it
     says on the tin. */
  const tp = propTileBox('tecpro');
  check('a TecPro module is exactly the 4 m it claims',
    Math.abs(tp.hz * 2 - 4) < 0.01, `${(tp.hz * 2).toFixed(3)} m`);
  const run = PREFABS_BY_KEY.get('tecpro_run');
  check('and a run of them tiles without a seam or an overlap', !!run);
  if (run) {
    const parts = instantiatePrefab(run, { x: 0, y: 0, z: 0 }, 0, (i) => `tp${i}`);
    const zs = parts.map((pp) => pp.p[2]).sort((a, b) => a - b);
    let worst = 0;
    for (let i2 = 1; i2 < zs.length; i2++) worst = Math.max(worst, Math.abs(zs[i2] - zs[i2 - 1] - 4));
    check('  at exactly one module apart', worst < 0.01, `${worst.toFixed(3)} m out`);
  }
  // The deck has to clear anything that can leave the ground on four wheels.
  const parts = LIBRARY.find((d) => d.key === 'bridge').build();
  const bb = new THREE.Box3();
  for (const part of parts) {
    part.geometry.computeBoundingBox();
    bb.union(part.geometry.boundingBox);
  }
  check('the bridge spans a circuit and its verges',
    bb.max.z - bb.min.z >= 24, `${(bb.max.z - bb.min.z).toFixed(1)} m`);
  check('and its deck is out of a car\'s reach', bb.min.y > -0.01 && bb.max.y >= 5.5,
    `${bb.min.y.toFixed(2)} to ${bb.max.y.toFixed(2)} m`);
}
...

{
  const CARDS = [
    ...
    ['tree_scrub_2d', 'scrub'],
    ['tree_birch_2d', 'birch'],
    ['tree_willow_2d', 'willow'],
    ['tree_autumn_2d', 'autumn'],
    ['tree_cypress_2d', 'cypress'],
    ['tree_fir_2d', 'fir'],
  ];
  ...
  for (const [key, species] of CARDS) {
     ...
     */
    const grid = TREE_SHEET_TILES;
    const col = spec.tile % grid;
    const row = (spec.tile / grid) | 0;
    const uv = g.getAttribute('uv');
    let stray = 0;
    for (let i = 0; i < uv.count; i++) {
      const u = uv.getX(i);
      const v = uv.getY(i);
      const inU = u >= col / grid - 1e-6 && u <= (col + 1) / grid + 1e-6;
      const inV = v >= (grid - 1 - row) / grid - 1e-6 && v <= (grid - row) / grid + 1e-6;
      if (!inU || !inV) stray++;
    }
  ...
  check('all nine species share one material',
   ...
   */
  {
    ...
    check('and every crown on it has sky cut through it',
      holes.length === 7 && channels.length === 7 && holes.every((h) => h > 20) && channels.every((c) => c > 2),
