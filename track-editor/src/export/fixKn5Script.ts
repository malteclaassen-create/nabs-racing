/**
 * fix_kn5.py -- shipped in the export ZIP next to blender_to_fbx.py.
 *
 * ksEditor writes a kn5 that works but leaves two gaps the editor cannot
 * close from the FBX side:
 *
 *  1. Props are exported as 1PROP_* (purely visual) because vanilla AC's
 *     culling drops small renderable surface-named meshes depending on the
 *     view direction. Collision therefore needs invisible 1WALL_* duplicates,
 *     the same pattern Kunos tracks use -- and the renderable flag only
 *     exists inside the kn5.
 *  2. ksEditor derives the ks* lighting constants from the FBX material
 *     colours, which tends towards a nearly unlit track.
 *
 * The script is plain stdlib Python. It refuses to write unless it can parse
 * the file back to the exact last byte, and it backs the input up first.
 */
export const FIX_KN5_SCRIPT = `"""
