# Discord bot for the server tokens
...
- who invited a new member (so the inviter gets their tokens automatically)
...
TOKEN_KEY=       Admin -> Tokens -> Discord bot
```
...
The bot can run before the tokens are switched on for members. Good idea actually: start it a month early and the multipliers are already filled when the feature goes live. It can't count backwards.
