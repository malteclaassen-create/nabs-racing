...
// is a 4px dot in a dark header, which on a phone is genuinely easy to miss;
// the movement is what gets noticed. It stops the moment the panel opens,
// because by then it has done its job and would just be fidgeting.
function BellIcon({ ringing = false }) {
  return (
    <svg viewBox="0 0 24 24" className={`h-[18px] w-[18px] ${ringing ? "bell-ring" : ""}`} fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" aria-hidden="true">
...
export default function NotificationBell({ className = "" }) {
  ...
  return (
    <div ref={wrapRef} className="relative">
      ...
      >
        <BellIcon ringing={unread > 0 && !open} />
        {unread > 0 && (
          <span
            aria-hidden
      ...
      {open && (
        <>
          ...
          <div className="notif-pop absolute right-0 top-full z-dropdown mt-2 w-80 max-w-[calc(100vw-5.5rem)] overflow-hidden rounded-xl border border-border bg-card shadow-2xl shadow-ink/20 lg:max-w-[calc(100vw-1.5rem)]">
