...
function DiscordEvents() {
  ...
  const [error, setError] = useState(null);
  const [busy, setBusy] = useState(false);
  const [event, setEvent] = useState({ number: "", track: "", date: "" });
  ...
  async function createEvent(e) {
    ...
    setBusy(true); setError(null); setMsg(null);
    try {
      await api.createEvent({ number: Number(event.number), track: event.track, date: event.date || null });
      setMsg(`Round ${event.number} created.`);
      setEvent({ number: "", track: "", date: "" });
      reloadRaces();
    } catch (err) { setError(err.message); } finally { setBusy(false); }
  }
    ...
    <div className="grid gap-6 lg:grid-cols-2">
      ...
      <div className="space-y-6">
        <form onSubmit={createEvent} className="card space-y-3 p-5">
          <CardHead eyebrow="Schedule" title="Create upcoming race" />
          <div className="grid grid-cols-2 gap-3">
            ...
            <input className="input" placeholder="Track" value={event.track}
          ...
          <button className="btn-primary w-full" disabled={busy}>Create race</button>
        </form>
          ...
          <ul className="divide-y divide-border">
            {upcoming.map((r) => (
              <li key={r.id} className="flex items-center justify-between py-2 text-sm">
                <span className="font-semibold text-dark">Round {r.number} — {r.track}</span>
