...
function DesignSwatch({ item, driver, appearance, stats }) {
  ...
  return <ProfileAppearance driver={sample} appearance={look} className={`shop-design-swatch shop-design-swatch--${item.slot}`}>
    {item.slot === "theme" ? <div className="shop-palette"><span /><span /><span /><span /></div> : item.slot === "nameplate" ? <span className="profile-driver-name">{driver?.name || "Driver"}</span> : <div className="profile-stats"><div><span>WINS</span><strong className="font-display">{stats?.wins ?? "—"}</strong></div><div><span>PODIUMS</span><strong className="font-display">{stats?.podiums ?? "—"}</strong></div><div><span>POLES</span><strong className="font-display">{stats?.polePositions ?? "—"}</strong></div></div>}
  </ProfileAppearance>;
}
...
export default function ProfileStyle() {
  ...
  return <div className={`profile-shop ${pageOnly ? "profile-shop--page" : "space-y-6"}`} style={{ "--profile-team": driver?.team?.color || "#638daf" }}>
      ...
      <section className="shop-catalog" aria-label="Profile designs">
        ...
        <div className="shop-items" data-category={slot}>
          ...
          {items.map(item => <div className="shop-design-option" key={item.id}><button type="button" className="shop-item" aria-label={`Preview ${item.name}`} aria-pressed={draft[slot] === item.id} disabled={editingDisabled} onClick={() => choose(item)}>
            ...
            <span className="shop-item-caption"><strong>{item.name}</strong><span>{profileItemPrice(catalogue, item, owned) === 0 ? "Owned" : `${profileItemPrice(catalogue, item, owned)} pts`}</span></span>
        ...
        {locked && <div className="shop-unlock-summary"><span>{missing.length} {missing.length === 1 ? "design" : "designs"} to unlock · {missingCost.toLocaleString()} pts</span>{missing.map(item => <button key={item.id} type="button" onClick={() => changeCategory(item.slot)}>{item.name}</button>)}</div>}{notice && <p role="status" className="shop-message">{notice}</p>}
      </section>
    ...
    <section className="shop-real-preview" aria-label="Profile preview">
      {(roster.error || profile.error) && !me?.isLinked && <div role="alert" className="shop-message shop-message--error">{profile.error || roster.error}<button className="btn-secondary ml-3" onClick={() => { roster.reload(); profile.reload(); }}>Retry</button></div>}
