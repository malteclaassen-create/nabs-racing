...
// ---------------------------------------------------------------------------
// /settings — the site's own preferences, as a page.
//
// This used to be a drawer sliding in from the right, opened from the bell and
// from a row in the profile's navigation. A drawer is the right shape for
// something you do TO the page you are on; settings are not that. They have an
// address worth linking to and bookmarking, they are read on a phone where a
// drawer is a page anyway, and there was no room in one for a setting that
// needs a sentence of explanation — which the first new setting here does.
//
// Everything on this page is stored in this browser (localStorage), not on the
...
// reason — a phone and a desktop are used differently. The page says so, so
// nobody wonders why their laptop did not follow.
// ---------------------------------------------------------------------------
  ...
  return (
    <div className="content-in mx-auto max-w-2xl space-y-6">
      <PageHeader
        eyebrow="Preferences"
        title="Settings"
        subtitle="Kept in this browser, on this device — a phone and a desktop each keep their own."
      />
      ...
      {/* Account: the sign-out that used to live in the drawer. */}
      {isLoggedIn && (
