...
import { applyPenalties, getDriverResultPoints, stampFastestLapBonus, stampPointsMultiplier } from "../services/pointsCalculator.js";
import { readManualFastestLaps } from "./raceHonours.js";
import { roundMultipliers } from "../services/standingsService.js";
...
async function classification(prisma, race, limit) {
  ...
  const mult = await roundMultipliers(prisma, [race.id]).catch(() => new Map());
  return stampPointsMultiplier(stampFastestLapBonus(applyPenalties(rows), bonus, manual), mult)
